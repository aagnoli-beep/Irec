...
class Mandante(TenantScoped, Base):
    ...
    __table_args__ = (UniqueConstraint("tenant_id", name="uq_mandante_tenant"),)

    ragione_sociale: Mapped[str] = mapped_column(String(255))
    partita_iva: Mapped[str] = mapped_column(String(20))
    pacchetto: Mapped[Pacchetto] = mapped_column(String(16))
...
class ClienteFinale(TenantScoped, Base):
    ...
    mandante_id: Mapped[str] = mapped_column(
        ForeignKey("mandante.id", ondelete="CASCADE"), index=True
    )
    ...
    canali_opt_out: Mapped[list] = mapped_column(JSON, default=list)
...
class Posizione(TenantScoped, Base):
    ...
    cliente_id: Mapped[str] = mapped_column(
        ForeignKey("cliente_finale.id", ondelete="CASCADE"), index=True
    )
    stato: Mapped[StatoPosizione] = mapped_column(
        String(16), default=StatoPosizione.APERTA
    )
...
class Fattura(TenantScoped, Base):
    ...
    posizione_id: Mapped[str] = mapped_column(
        ForeignKey("posizione.id", ondelete="CASCADE"), index=True
    )
    cliente_id: Mapped[str] = mapped_column(
        ForeignKey("cliente_finale.id", ondelete="CASCADE"), index=True
    )
    ...
    stato: Mapped[StatoFattura] = mapped_column(
        String(16), default=StatoFattura.GESTIONE
    )
...
class FlussoStep(TenantScoped, Base):
    ...
    __table_args__ = (
        UniqueConstraint("flusso_id", "ordine", name="uq_step_flusso_ordine"),
    )

    flusso_id: Mapped[str] = mapped_column(
        ForeignKey("flusso.id", ondelete="CASCADE"), index=True
    )
    ordine: Mapped[int] = mapped_column()
    offset_giorni: Mapped[int] = mapped_column()
    canale: Mapped[Canale] = mapped_column(String(16))
...
class Comunicazione(TenantScoped, Base):
    ...
    __table_args__ = (
        ...
        # anche dopo un ricalcolo dello scheduler (PRD 5.3).
        UniqueConstraint(
    ...
    fattura_id: Mapped[str] = mapped_column(
        ForeignKey("fattura.id", ondelete="CASCADE"), index=True
    )
    step_id: Mapped[str | None] = mapped_column(
        ForeignKey("flusso_step.id", ondelete="SET NULL")
    )
    canale: Mapped[Canale] = mapped_column(String(16))
    template: Mapped[str] = mapped_column(String(128))
    ...
    stato: Mapped[StatoComunicazione] = mapped_column(
        String(16), default=StatoComunicazione.PROGRAMMATA
    )
    operatore: Mapped[str | None] = mapped_column(String(64))
...
class Pagamento(TenantScoped, Base):
    ...
    origine: Mapped[OriginePagamento] = mapped_column(String(16))
...
class AuditLog(TenantScoped, Base):
    ...
    __table_args__ = (Index("ix_audit_tenant_entita", "tenant_id", "entita", "entita_id"),)

    tipo: Mapped[TipoEvento] = mapped_column(String(32))
    entita: Mapped[str] = mapped_column(String(32))
    entita_id: Mapped[str] = mapped_column(String(32))
